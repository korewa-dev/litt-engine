...
impl litt_ecs::System for MovementSystem {
    ...
    fn update(&mut self, world: &mut World, _dt: f32) {
        // Collect data first to avoid overlapping borrows of `world`
...
impl litt_ecs::System for CameraSystem {
    ...
    fn update(&mut self, world: &mut World, _dt: f32) {
        let player_data = world
            .query_entities_with::<Player, Transform>()
            .into_iter()
            .filter_map(|e| world.get_component::<Transform>(e).cloned())
            .next();

        let Some(player_transform) = player_data else { return };

        let camera_updates: Vec<_> = world
            .query_entities::<Camera>()
            .into_iter()
            .filter_map(|camera_entity| {
                let camera = world.get_component::<Camera>(camera_entity)?.clone();
                Some((camera_entity, camera))
            })
            .collect();

        for (camera_entity, camera) in camera_updates {
...
impl litt_ecs::System for LightSystem {
    ...
    fn update(&mut self, world: &mut World, dt: f32) {
        let light_updates: Vec<_> = world
            .query_entities::<Light>()
            .into_iter()
            .filter_map(|entity| {
                let light = world.get_component::<Light>(entity)?.clone();
                Some((entity, light))
            })
            .collect();

        for (entity, light) in light_updates {
...
impl litt_ecs::System for PhysicsTransformSyncSystem {
    ...
    fn update(&mut self, world: &mut World, _dt: f32) {
        let syncs: Vec<_> = world
            .query_entities_with::<PhysicsTransform, Transform>()
            .into_iter()
            .filter_map(|entity| {
                let phys_tr = world.get_component::<PhysicsTransform>(entity)?.clone();
                let render_tr = world.get_component::<Transform>(entity)?.clone();
                Some((entity, phys_tr, render_tr))
            })
            .collect();

        for (entity, phys_tr, render_tr) in syncs {
            world.add_component(entity, Transform {
