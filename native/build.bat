@echo off
rem Build littcore + littcli + game + dither3d_demo with gcc/g++ (llvm-mingw). Usage:
rem   build.bat               -> build lib objects + littcli.exe + game.exe
rem   build.bat test          -> build and run unit tests
rem   build.bat dither3d_demo -> build dither3d_demo.exe
setlocal
cd /d "%~dp0"
