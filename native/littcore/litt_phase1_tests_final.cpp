// Phase 1 Tests - Simplified and Working (Final Version)
#include <iostream>
#include <cassert>
#include <cstring>
#include <vector>
...
class EventManager {
public:
    void subscribe_int(std::function<void(int)> callback) {
        int_callbacks_.push_back(callback);
    }
    
    void emit_int(int value) {
        for (auto& cb : int_callbacks_) {
            cb(value);
        }
    }

private:
    std::vector<std::function<void(int)>> int_callbacks_;
};
