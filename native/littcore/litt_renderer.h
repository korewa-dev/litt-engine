...
struct RenderPass {
    ...
    std::function<void(const Camera&)> render;
};
...
public:
    ...
    virtual void draw_mesh(const Mesh& mesh, const Mat4f& transform, const Material& material) = 0;
...
struct SceneNode {
    ...
    SceneNode* find_child(const std::string& name) {
...
public:
    ...
    SceneNode* find_node(const std::string& name) {
...
public:
    ...
    void clear(Vec3f color, float depth, uint32_t stencil) override {
        // Clear buffers
    }
    ...
    void draw_mesh(const Mesh& mesh, const Mat4f& transform, const Material& material) override {
        // Draw mesh with transform and material
    }
    
    void draw_line(const Vec3f& start, const Vec3f& end, Vec3f color) override {
        // Draw line (for debug visualization)
    }
    
    void draw_gizmo(const Vec3f& pos, const Vec3f& rot, float scale) override {
    ...
    void render_node(const SceneNode& node) {
...
private:
    ...
    bool init_dx12(uint32_t width, uint32_t height) {
        // DirectX 12 initialization
        return false;
    }
    
    bool init_opengl(uint32_t width, uint32_t height) {
        // OpenGL initialization
        return false;
    }
    
    bool init_metal(uint32_t width, uint32_t height) {
