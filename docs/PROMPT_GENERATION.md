...
| Render | [RenderSystem](RenderSystem.md) | 🔲 New | ECS→GPU draw pipeline, frame graph, shader compilation |
...
Must cover:
- The ECS→GPU pipeline: `Renderable` component → command buffer recording → draw dispatch
