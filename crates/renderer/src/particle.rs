    ...
    pub fn emit(&mut self) -> Particle {
        let position = match self.emitter_type {
            ...
            EmitterType::Box(min, max) => {
                Vec3::new(
                    rng_f32() * (max.0 - min.0) + min.0,
                    rng_f32() * (max.1 - min.1) + min.1,
                    rng_f32() * (max.2 - min.2) + min.2,
                )
            }
            EmitterType::Sphere(center, radius) => {
                let theta = rng_f32() * 2.0 * std::f32::consts::PI;
                let phi = rng_f32() * std::f32::consts::PI;
                Vec3::new(
            ...
            EmitterType::Cone(origin, direction, angle) => {
                let theta = rng_f32() * 2.0 * std::f32::consts::PI;
                let sin_angle = (angle * rng_f32()).sin();
                let cos_angle = (angle * rng_f32()).cos();
                let offset = Vec3::new(
                    sin_angle * theta.cos(),
                    sin_angle * theta.sin(),
                    cos_angle,
        ...
        let velocity = {
            let speed = self.params.initial_speed + (rng_f32() - 0.5) * self.params.speed_variance;
            let dir = Vec3::new(
                rng_f32() * 2.0 - 1.0,
                rng_f32() * 2.0 - 1.0,
                rng_f32() * 2.0 - 1.0,
        ...
        let color = [
            self.params.color[0] + (rng_f32() - 0.5) * self.params.color_variance[0],
            self.params.color[1] + (rng_f32() - 0.5) * self.params.color_variance[1],
            self.params.color[2] + (rng_f32() - 0.5) * self.params.color_variance[2],
            self.params.color[3] + (rng_f32() - 0.5) * self.params.color_variance[3],
        ];

        let size = self.params.size + (rng_f32() - 0.5) * self.params.size_variance;
...
// -- deterministic fallback RNG (no external rand dependency) --
use std::sync::atomic::{AtomicU64, Ordering};

static RNG_STATE: AtomicU64 = AtomicU64::new(0x9E3779B97F4A7C15);

/// Cheap thread-safe pseudo-random f32 in [0, 1).
pub fn rng_f32() -> f32 {
