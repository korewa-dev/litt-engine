...
impl OctreeNode {
    ...
    /// Returns true when the child node accepted the object.
    pub fn insert(&mut self, bbox: Bbox, object_id: usize) -> bool {
        // If node has children, try to insert into children
        if let Some(ref mut children) = self.children {
            let idx = Self::subdivide_index_of(self.bounds.center(), bbox.center());
    ...
    /// Get subdivide index for a point relative to a center
    fn subdivide_index_of(center: Vec3, point: Vec3) -> usize {
        let mut idx = 0;
...
impl SpatialHash {
    ...
    pub fn get_pairs(&self) -> Vec<(usize, usize)> {
        ...
        let mut seen_u64: Vec<u64> = Vec::new();

        for (cell_id, objects) in &self.grid {
            for i in 0..objects.len() {
                for j in (i + 1)..objects.len() {
                    let pair = if objects[i] < objects[j] {
                        ((objects[i] as u64) << 32) | objects[j] as u64
                    } else {
                        ((objects[j] as u64) << 32) | objects[i] as u64
                    };
                    if !seen_u64.contains(&pair) {
                        seen_u64.push(pair);
