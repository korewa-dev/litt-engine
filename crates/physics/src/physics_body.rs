...
mod tests {
    ...
    fn test_capsule_bounding_radius() {
        ...
        let expected: f32 = (0.25f32 + 1.0f32).sqrt();
        assert!((radius - expected).abs() < 1e-6);
    }

    #[test]
    fn test_physics_body_gpu_layout() {
        // Verify the struct keeps its 16-byte GPU alignment.
        // Fields: shape_type, mass, inv_mass, 2xVec3 velocity/angular,
        // damping x2, friction, restitution, layer, is_trigger, gravity_scale,
        // shape_data[4] => 80 bytes with align(16).
        assert_eq!(std::mem::size_of::<PhysicsBody>(), 80);
...
impl Default for ColliderShape {
    fn default() -> Self {
        ColliderShape::AABB { half_extent: Vec3::new(0.5, 0.5, 0.5) }
    }
}
