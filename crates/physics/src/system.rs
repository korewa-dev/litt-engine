...
use litt_math::Vec3;
use ash::{vk, Device};
use litt_vulkan::{GpuAllocator, ComputePipeline, create_compute_pipeline};
...
impl PhysicsSystem {
    ...
    fn create_descriptor_set_layouts(
        ...
        unsafe {
            let make_binding = || vk::DescriptorSetLayoutBinding {
                binding: 0,
            ...
            let body_bindings = [make_binding()];
            let body_layout = vk::DescriptorSetLayoutCreateInfo {
                binding_count: body_bindings.len() as u32,
                p_bindings: body_bindings.as_ptr(),
                ..Default::default()
            };

            // Binding 1: Transform buffer
            let transform_bindings = [make_binding()];
            let transform_layout = vk::DescriptorSetLayoutCreateInfo {
                binding_count: transform_bindings.len() as u32,
                p_bindings: transform_bindings.as_ptr(),
                ..Default::default()
            };
...
pub struct GPUPhysicsPipeline {
    ...
    pub body_buffer: Option<(vk::Buffer, litt_vulkan::Allocation)>,
    /// Transform buffer (GPU memory, written by CPU, read by shader)
    pub transform_buffer: Option<(vk::Buffer, litt_vulkan::Allocation)>,
    /// Output transform buffer (double-buffered for readback)
    pub output_transform_buffer: Option<(vk::Buffer, litt_vulkan::Allocation)>,
    /// Grid buffer for spatial hash (GPU)
    pub grid_buffer: Option<(vk::Buffer, litt_vulkan::Allocation)>,
...
impl GPUPhysicsPipeline {
    ...
    pub fn reserve(
        ...
        let (body_buf, body_alloc) = allocator.allocate_buffer(
            ...
            litt_vulkan::AllocFlags::DEVICE_LOCAL,
    ...
    pub unsafe fn upload_bodies(
        ...
        if let Some((buf, alloc)) = &mut self.body_buffer {
...
impl std::fmt::Debug for GPUPhysicsPipeline {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GPUPhysicsPipeline")
            .field("broadphase_ready", &self.broadphase_pipeline.is_some())
            .field("integrate_ready", &self.integrate_pipeline.is_some())
