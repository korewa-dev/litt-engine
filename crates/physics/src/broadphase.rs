...
pub struct Bvh {
    ...
    /// Index of the root node in `nodes` (0 when empty)
    pub root: usize,
}
...
impl Bvh {
    ...
    pub fn build(&mut self, aabbs: &[(Vec3, Vec3)]) {
        ...
        self.root = self.build_sah(aabbs, 0, count);
    ...
    fn build_sah(&mut self, aabbs: &[(Vec3, Vec3)], start: usize, end: usize) -> usize {
        ...
        if count <= 4 {
            ...
            // Fold the leaves into a binary tree so every leaf stays reachable
            // from the returned root (traversal is strictly binary).
            let mut acc = leaf_indices[0];
        ...
        for axis in 0..3 {
            ...
            for (i, (body_idx, pos)) in sorted_by_axis.iter().copied().enumerate() {
    ...
    pub fn find_overlaps(&self, target_aabb_min: Vec3, target_aabb_max: Vec3, results: &mut Vec<usize>) {
        ...
        self.traverse_overlaps(self.root, target_aabb_min, target_aabb_max, results);
    }
