...
unsafe fn build_rdna_pipeline(
    ...
    let pipelines = device
        ...
        .map_err(|(_, e)| format!("RDNA compute pipeline: {:?}", e))?;
...
impl RDNAPhysicsTier {
    ...
    pub fn dispatch_wave32(
        ...
        unsafe {
            pipeline.device.cmd_bind_pipeline(
                command_buffer,
                vk::PipelineBindPoint::COMPUTE,
                pipeline.pipeline,
            );
            pipeline.device.cmd_bind_descriptor_sets(
                command_buffer,
                vk::PipelineBindPoint::COMPUTE,
                pipeline.layout,
                0,
            ...
            pipeline.device.cmd_push_constants(
                command_buffer,
                pipeline.layout,
            ...
            pipeline.device.cmd_dispatch(command_buffer, groups, 1, 1);
        }
        Ok(())
    }
    ...
    pub fn allocate_wave32_descriptor_set(&self) -> Result<vk::DescriptorSet, String> {
        let pipeline = self.wave32_pipeline.as_ref()
            .ok_or("Wave32 pipeline not initialized")?;
        unsafe {
            let layouts = [pipeline.desc_layout];
...
/// Compute AABB half-extents for all physics bodies (CPU-side, for upload).
///
/// Shape encoding: 0=AABB(half_extent xyz), 1=Sphere(r), 2=Capsule(r, half_h).
pub fn compute_aabbs(bodies: &[crate::physics_body::PhysicsBody]) -> Vec<Vec3> {
    bodies
        .iter()
        .map(|body| match body.shape_type {
            0 => Vec3::new(body.shape_data[0], body.shape_data[1], body.shape_data[2]),
            1 => Vec3::new(body.shape_data[0], body.shape_data[0], body.shape_data[0]),
            _ => Vec3::new(body.shape_data[0], body.shape_data[1], body.shape_data[0]),
        })
        .collect()
}

/// Compute the maximum half-extent across all bodies (bounding sphere radius hint).
pub fn compute_aabb_max(bodies: &[crate::physics_body::PhysicsBody]) -> Vec3 {
    let mut max = Vec3::new(f32::NEG_INFINITY, f32::NEG_INFINITY, f32::NEG_INFINITY);
    for half in compute_aabbs(bodies) {
        max.0 = max.0.max(half.0);
        max.1 = max.1.max(half.1);
        max.2 = max.2.max(half.2);
    }
    max
}
...
impl std::fmt::Debug for RdnaComputePipeline {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RdnaComputePipeline")
