...
impl DataType {
    ...
    pub fn is_supported(&self, backend: &BackendKind) -> bool {
        match self {
            ...
            Self::Float16 => matches!(backend, BackendKind::Gpu(_) | BackendKind::Npu(_) | BackendKind::Cpu(_)),
            Self::Int8 => matches!(backend, BackendKind::Npu(_) | BackendKind::Cpu(_)),
            Self::Uint8 => matches!(backend, BackendKind::Npu(_) | BackendKind::Cpu(_)),
            Self::Int32 => true,
            Self::Bf16 => matches!(backend, BackendKind::Gpu(_) | BackendKind::Npu(_)),
        }
    }
}
...
impl Tensor {
    ...
    pub fn empty(shape: Shape, data_type: DataType) -> Self {
        Self {
            shape,
            data_type,
            data: vec![0u8; shape.size_bytes(data_type)],
