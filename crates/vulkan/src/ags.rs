...
impl GpuProperties {
    pub fn from_vulkan(instance: &Instance, physical_device: vk::PhysicalDevice) -> Self {
        ...
        unsafe {
            ...
            // vk deviceName is [i8; VK_MAX_PHYSICAL_DEVICE_NAME_SIZE]
            let raw = &device_props.device_name;
            let bytes: Vec<u8> = raw.iter().map(|&c| c as u8).collect();
            let name_len = bytes.iter().position(|&b| b == 0).unwrap_or(bytes.len());
            let name_len = name_len.min(props.name.len());
            props.name[..name_len].copy_from_slice(&bytes[..name_len]);
            ...
            let total_vram = device_mem_props.memory_heaps
                .iter()
                .take(device_mem_props.memory_heap_count as usize)
                .map(|heap| heap.size)
                .sum::<u64>();
            props.vram_mb = (total_vram / (1024 * 1024 * 1024)) as u32;

            let name_str = String::from_utf8_lossy(&props.name[..name_len])
                .to_lowercase();
...
impl GpuManager {
    ...
    pub fn select_best(&mut self) -> Result<&GpuProperties, String> {
        ...
        self.selected = Some(self.gpus[best_idx].clone());
...
impl AmgInfo {
    pub fn from_gpu(gpu: &GpuProperties) -> Self {
        ...
        let vendor_name = match &gpu.vendor {
            ...
            GpuVendor::Other(name) => name.clone(),
            _ => "Unknown".to_string(),
        };
        
        let gpu_name = String::from_utf8_lossy(
            &gpu.name[..gpu.name.iter().position(|&b| b == 0).unwrap_or(gpu.name.len())],
        )
        .to_string();
