...
use ash::{vk, Instance};
use crate::ags::{GpuManager, GpuSelectionCriteria};

/// Required Vulkan extensions for all platforms
/// Supports: AMD (RADV/AMDVLK), Moore Threads (MUSA), Intel (Arc/DDR Xe)
const REQUIRED_EXTENSIONS: &[&str] = &["VK_KHR_surface", "VK_KHR_swapchain"];
...
pub fn enumerate_adapters(
    ...
    for device in devices {
        let info = unsafe { instance.get_physical_device_properties(device) };
        let vendor_id = info.vendor_id;

        // AMD vendor ID
        if vendor_id == 0x1002 {
            amd_devices.push(device);
        // Moore Threads vendor ID (0x1DD = 477)
...
pub fn is_compatible(
    ...
    for ext in required_extensions {
        ...
        if !device_extensions.iter().any(|e| {
            unsafe { std::ffi::CStr::from_ptr(e.extension_name.as_ptr()) == ext_name }
        }) {
            return false;
        }
    }
