...
impl Arena {
    ...
    pub fn alloc_typed<T>(&self, value: T) -> *mut T {
    ...
    fn new(chunk_size: usize, chunk_count: usize) -> Self {
        ...
        unsafe {
            ...
            for i in (0..chunk_count).rev() {
                ...
                let next = free;
                *(memory.add(offset) as *mut Option<usize>) = next;
                free = Some(offset);
            }
...
impl Pool {
    ...
    pub fn alloc(&mut self) -> *mut u8 {
        ...
        if let Some(block) = self.blocks.last_mut() {
            if let Some(offset) = block.free_list {
                let next = unsafe { *(block.memory.add(offset) as *mut Option<usize>) };
                block.free_list = next;
                self.allocation_count.fetch_add(1, Ordering::Relaxed);
                return unsafe { block.memory.add(offset) };
            }
        }
        ...
        if let Some(offset) = block.free_list {
            ...
            self.allocation_count.fetch_add(1, Ordering::Relaxed);
            unsafe {
                let next = *(block.memory.add(offset) as *mut Option<usize>);
                block.free_list = next;
            }
            return unsafe { block.memory.add(offset) };
        }
    ...
    pub fn free(&mut self, ptr: *mut u8) {
        for block in &mut self.blocks {
            ...
            if ptr_addr >= block_start && ptr_addr < block_end {
                ...
                unsafe {
                    *(ptr as *mut Option<usize>) = block.free_list;
                    block.free_list = Some(offset);
                }
