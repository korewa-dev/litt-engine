...
impl MusaGpuInfo {
    ...
    pub fn from_physical_device(physical_device: vk::PhysicalDevice, instance: &Instance) -> Self {
        unsafe {
            ...
            // Query compute features
            let wg_props = instance.physical_device_compute_replication_properties_ext(physical_device);
            let compute_units = wg_props.num_shaders;
            let max_wg_size = props.max_work_group_size[0];
            let max_grid_x = props.max_compute_work_group_counts[0];
...
unsafe fn build_musa_compute_pipeline(
    ...
    let shader_info = vk::ShaderModuleCreateInfo::builder().code(&spv).build();
...
impl MusaContext {
    ...
    pub fn new(instance: &Instance, physical_device: vk::PhysicalDevice) -> Result<Self, MusaError> {
        ...
        unsafe {
            ...
            let queue_families = instance.physical_device_queue_family_properties(physical_device);
            ...
            let exts = instance.enumerate_device_extension_properties(physical_device).unwrap_or_default();
