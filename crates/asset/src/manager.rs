...
impl Asset {
    pub fn handle(&self) -> AssetHandle {
        match self {
            Self::Model(m) => AssetHandle::from_path(&m.name, AssetType::Model),
    ...
    pub fn state(&self) -> AssetState {
        match self {
            Self::Model(m) if !m.meshes.is_empty() => AssetState::Loaded,
            Self::Model(_) => AssetState::Pending,
            Self::Texture(t) => t.state.clone(),
            Self::Shader(s) => s.state.clone(),
            _ => AssetState::Loaded,
        }
    }
}
...
impl AssetManager {
    ...
    pub fn get<T: 'static>(&self, handle: &AssetHandle) -> Option<&T> {
        ...
        match asset {
            Asset::Model(m) => (m.as_ref() as &dyn std::any::Any).downcast_ref::<T>(),
            Asset::Texture(t) => (t.as_ref() as &dyn std::any::Any).downcast_ref::<T>(),
            Asset::Shader(s) => (s.as_ref() as &dyn std::any::Any).downcast_ref::<T>(),
            Asset::Material(m) => (m.as_ref() as &dyn std::any::Any).downcast_ref::<T>(),
            Asset::Font(f) => (f.as_ref() as &dyn std::any::Any).downcast_ref::<T>(),
        }
    }

    /// Get a mutable reference to a loaded asset
    pub fn get_mut<T: 'static>(&mut self, handle: &AssetHandle) -> Option<&mut T> {
        let asset = self.assets.get_mut(handle)?;
        match asset {
            Asset::Model(m) => (m.as_mut() as &mut dyn std::any::Any).downcast_mut::<T>(),
            Asset::Texture(t) => (t.as_mut() as &mut dyn std::any::Any).downcast_mut::<T>(),
            Asset::Shader(s) => (s.as_mut() as &mut dyn std::any::Any).downcast_mut::<T>(),
            Asset::Material(m) => (m.as_mut() as &mut dyn std::any::Any).downcast_mut::<T>(),
            Asset::Font(f) => (f.as_mut() as &mut dyn std::any::Any).downcast_mut::<T>(),
        }
    }
